import { loadData, renderDetailInner, resolveRelated } from './data.js'
...
async function main() {
  try {
    const { byId, quotes } = await loadData()
    const q = id && byId[id]
    if (!q) {
      el.innerHTML = `<p class="err">未找到该金句。</p>`
      document.title = '未找到 · 一句话投资'
    } else {
      const related = resolveRelated(q, byId, quotes)
      el.innerHTML = renderDetailInner(q, related)
